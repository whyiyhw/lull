...
// 所有替换 / 新增素材都必须过这条流水线，产出直接落到 audio/ 供 index.html 加载。
//
// 用法：
//   node scripts/prep-audio.mjs                     # 处理 audio/raw/ 下所有音频 → audio/<同名>.mp3
//   node scripts/prep-audio.mjs in.wav lrain        # 单文件 → audio/lrain.mp3
...
function usage() {
  ...
  node scripts/prep-audio.mjs                  处理 audio/raw/ 全部 → audio/<同名>.mp3
...
function process1(input, name) {
  ...
  console.log(`✓ audio/${name}.mp3  (${(D - (seamless ? X : 0)).toFixed(1)}s · ${kb}KB · mono · ${opts.lufs}LUFS${seamless ? ` · xfade ${X.toFixed(1)}s` : ''})`);
  return true;
}
...
console.log(`prep-audio · ${jobs.length} 个素材 → audio/  (xfade ${opts.xfade}s / ${opts.lufs}LUFS / ${opts.bitrate})`);
let ok = 0;
